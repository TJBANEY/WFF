...
def upload(request):
	...
	try:
		...
		query = request.GET
		path = get_path(query.get('dir', ''))
		logger.info('upload - query: %s  path: %s', query, path)
		if 'pop' in query:
			is_popup = True
		else:
			is_popup = False
...
def _check_file(request):
	...
	try:
		...
		filearray_json = json.dumps(fileArray)
		logger.info('_check file returning: %s', filearray_json)
	except Exception as exc:
		logger.error(exc)
